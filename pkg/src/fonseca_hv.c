/**
 * fonseca_hv.c - Calculate dominated hypervolume using Fonseca et.al.s algorithm
 *
 * Authors:
 ...
 *  * Use R memory functions. (OME)
 *  * Fix memory leaks. (OME)
 *
 * Memory managment:
 *
 * This code does not clean up after itself. In most places explicit
 * free() calls were added. In some situations this was not deemed
 * worthwhile and instead R_alloc() was used. Memory allocated using
 * R_alloc() is freed when the main entry point returns control to the
 * R interpreter.
 */
 ...
 * avl.c - AVL tree
 *
 * Taken from by the original authors from the Linux kernel source
 * tree. Licensced under the GPL-2 or greater.
 */
#include <errno.h>
#define AVL_DEPTH
#define AVL_COUNT
...
inline static int avl_check_balance(avl_node_t *avlnode) {
  int d;
...
static dlnode_t * setup_cdllist(const double *data, const int d, const int n) {
  ...
  head->tnode  = (avl_node_t *)R_alloc(1, sizeof(avl_node_t));
  head->area   = (double *)R_alloc(d * (n+1), sizeof(double));
  head->vol    = (double *)R_alloc(d * (n+1), sizeof(double));
  ...
  for (i = 1; i <= n; i++) {
      ...
      head[i].tnode = (avl_node_t *)R_alloc(1, sizeof(avl_node_t));
