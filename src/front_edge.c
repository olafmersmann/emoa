...
SEXP do_which_points_on_edge(SEXP s_front) {
   ...
   * on return.
   */
  double *base_reference_point =
      (double *)R_alloc(number_of_objectives, sizeof(double));
  double *current_reference_point =
      (double *)R_alloc(number_of_objectives, sizeof(double));
  ...
  for (point = 0; point < number_of_points; ++point) {
    ...
    for (other_point = 0; other_point < number_of_points; ++other_point) {
      ...
      size_t last_dominated_objective = 0;
      for (objective = 0; objective < number_of_objectives; ++objective) {
     ...
     */
    for (objective = 0; objective < number_of_objectives; ++objective) {
      if (current_reference_point[objective] ==
          base_reference_point[objective]) {
        result[point] = TRUE;
        break;
      }
    }
