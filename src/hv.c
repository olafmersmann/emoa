...
static double hv_recursive(dlnode_t *list, int dim, int c, const double *ref,
                           double *bound) {
  /* ------------------------------------------------------
  ...
  if (dim > stop_dimension) {
    ...
    if (c > 1) {
      hyperv = p1->prev[dim]->vol[dim] +
               p1->prev[dim]->area[dim] * (p1->x[dim] - p1->prev[dim]->x[dim]);
      p1->vol[dim] = hyperv;
    } else {
      int i;
      p1->area[0] = 1;
  ...
  /* ---------------------------
    ...
    height = (c == 1) ? ref[2] - pp->x[2] : pp->next[2]->x[2] - pp->x[2];
    ...
    pp = pp->next[2];
    do {
      height = (pp == list->prev[2]) ? ref[2] - pp->x[2]
      ...
      if (pp->ignore >= 2)
        ...
        if (avl_search_closest(tree, pp->x, &tnode) <= 0) {
          ...
          nxt_ip = (tnode->next != NULL) ? (double *)(tnode->next->item) : ref;
        }
  ...
  /* special case of dimension 1 */
    ...
    exit(EXIT_FAILURE);
    */
    error("hv: UNREACHABLE CODE REACHED. Please report this to the package "
          "author.");
...
double fpli_hv(double *data, int d, int n, const double *ref) {
  ...
  tree = avl_alloc_tree((avl_compare_t)compare_tree_asc, (avl_freeitem_t)NULL);
